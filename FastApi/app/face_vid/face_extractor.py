...
def face_extractor(img):
    ...
    for i, face_landmarks in enumerate(get_landmarks(img), start=1):
        face_img_name = "face_%s" % file_name
        aligned_face_path = os.path.join("result_gan_vid", face_img_name)

        img = image_align(img, face_landmarks)

        img.save(aligned_face_path)
        img = "result_gan_vid/face_%s" % file_name
