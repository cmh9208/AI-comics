...
from cycle_gan.models import GeneratorResNet
from face_vid2vid.sync_batchnorm import DataParallelWithCallback
...
def face_vid_parser():
    ...
    parser.add_argument("--driving_video", default='face_vid2vid/asset/driving/5.mp4', help="path to driving video")
...
def create_fake_img_and_vid(img):
    ...
    # (이 시점에선 애니 이미지이기 때문에 애니 얼굴 인식하기)
    opt = face_vid_parser()
    source_image = img
    ...
    imageio.v2.mimsave(result_vid_name, [img_as_ubyte(frame) for frame in predictions], fps=fps)
    sys.stdout.write('\r가짜 이미지와 영상 생성이 완료 되었습니다.')

if __name__ == '__main__':
    remove_memory_cash()
    img = "user_image/4.jpg"
